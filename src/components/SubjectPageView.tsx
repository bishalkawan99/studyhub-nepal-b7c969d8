...
export function SubjectPageView({ subject, classId }: { subject: Subject; classId: string }) {
        ...
        <header className="mt-4 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 sm:flex sm:flex-wrap sm:justify-between">
          ...
          <div className="flex shrink-0 gap-2">
            <Link
              ...
              params={{ classSlug: classSlug(classId), subjectSlug: subject.slug }}
              className="hidden items-center gap-2 rounded-xl brand-gradient px-4 py-2.5 text-xs font-semibold text-primary-foreground sm:inline-flex"
            >
              <ListChecks className="h-4 w-4" /> Practice MCQs
            </Link>
            <button
              onClick={() => {
            ...
            {tab === "MCQs" ? (
              <div className="mt-6 rounded-2xl border border-dashed border-border p-6 text-center">
                ...
                <p className="mt-3 text-sm font-semibold">
                  Chapter {chapter + 1} MCQ practice with timer, explanations and instant score
                </p>
                <Link
                  to="/$classSlug/$subjectSlug/mcqs/$chapter"
                  params={{
                    classSlug: classSlug(classId),
                    subjectSlug: subject.slug,
                    chapter: `chapter-${chapter + 1}`,
                  }}
                  className="mt-4 inline-flex rounded-full brand-gradient px-5 py-2.5 text-sm font-semibold text-primary-foreground"
                >
                  Start chapter quiz
                </Link>
              </div>
